...
def init(vault_dir: str):
    ...
    # 5. Install vault-search skill globally
    _install_skill()
    click.echo("vault-search skill installed globally.")
...
SKILL_SOURCE = Path(__file__).parent.parent.parent / ".claude" / "skills" / "vault-search" / "SKILL.md"


def _install_skill():
    """Copy vault-search skill to ~/.claude/skills/ for global availability."""
    ...
    if SKILL_SOURCE.exists():
        ...
        # Fallback: write skill inline if source not found (e.g. pip install)
        dest.write_text("""---
name: vault-search
...
def _uninstall_skill():
    """Remove vault-search skill from ~/.claude/skills/."""
...
def uninstall():
    ...
    _uninstall_skill()
    click.echo("vault-search skill removed.")
